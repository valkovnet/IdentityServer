...
certutil -f -addstore Root "%~dp0certs\identity.com.cer"
...
%cscript% /nologo %capicompath% import -l LM "%~dp0certs\localhost.pfx" "qwerty"

%cscript% /nologo %capicompath% import -l LM "%~dp0certs\identity.com.pfx" "qwerty"
...
@if "%IsW7%" == "true" (
  ...
  "%~dp0winhttpcertcfg.exe" -g -c LOCAL_MACHINE\My -s identity.com -a "IIS_IUSRS"
	set IsWinClient=true
) else (
	"%~dp0winhttpcertcfg.exe" -g -c LOCAL_MACHINE\My -s localhost -a "NETWORK SERVICE"
  "%~dp0winhttpcertcfg.exe" -g -c LOCAL_MACHINE\My -s identity.com -a "NETWORK SERVICE"
)
